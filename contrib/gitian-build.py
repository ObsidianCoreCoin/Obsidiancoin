...
def setup():
    ...
    if not os.path.isdir('obsidian'):
        subprocess.check_call(['git', 'clone', 'https://github.com/ObsidianCoreCoin/obsidiancoin.git'])
...
def build():
    ...
    os.makedirs('obsidian-binaries/' + args.version, exist_ok=True)
    ...
    subprocess.check_call(['make', '-C', '../obsidian/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/ObsidianCoreCoin/obsidiancoin', help='Specify the URL of the repository. Default is %(default)s')
    ...
    if args.pull:
        ...
        os.chdir('../gitian-builder/inputs/obsidian')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
