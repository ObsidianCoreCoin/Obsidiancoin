...
uint256 hashGenesisBlock("0xa30c66307b669f298e06c31f1acc57cc4375eeeaf532c5baf620677f5ef05ee5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Obsidian: starting difficulty is 1 / 2^12
...
{
    // Obsidian: IsDust() detection disabled, allows any valid dust to be relayed.
        ...
        if (fTxIndex) {
            ...
            if (pblocktree->ReadTxIndex(hash, postx)) {
                ...
                try {
                    ...
                    return error("%s() : deserialize or I/O error", __PRETTY_OBSCTION__);
                }
                hashBlock = header.GetHash();
                if (txOut.GetHash() != hash)
                    return error("%s() : txid mismatch", __PRETTY_OBSCTION__);
                return true;
            }
        }
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Obsidian: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 24 * 60 * 60; // Obsidian: 3.5 days
static const int64 nTargetSpacing = 1.5 * 60; // Obsidian: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Obsidian: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Obsidian: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8fcc4675019e5de0aa5eb8a83589c8733cf4ae424cb5ebdce593a426b559745d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Daddy's Little Angel 04/08/2017 Eva.D";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("044094ddff442d7886494fc63759595ec5e55a6cdde23221891f840f7fd509a8c8a07524166b6cd1324c97a613127fb3bbc224ff292044be9a2238b2457ac5d9fe") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1700145761;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085800396;

        if (fTestNet)
        {
            block.nTime    = 1700145742;
            block.nNonce   = 387731351;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x535e3894b42c97c8567afddebc0a6a7fc4c8f6b121f2c6d043f572ed3624916c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Obsidian: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ObsidianMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ObsidianMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ObsidianMiner(CWallet *pwallet)
{
    printf("ObsidianMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ObsidianMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ObsidianMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ObsidianMiner, pwallet));
}
