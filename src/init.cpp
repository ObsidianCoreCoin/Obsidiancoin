 ...
 */
static const char* BITCOIN_PID_FILENAME = "obsidiand.pid";
...
{
    const std::string URL_WEBSITE = "<https://obsidiancoin.org>";
    const std::string URL_DISCORD = "<https://discord.gg/BkzeBrWRDP>";
    const std::string URL_EXPLORER = "<https://explorer.obsidiancoin.org/>";
    const std::string URL_YOUTUBE = "<https://www.youtube.com/@ObsidianCoin>";

       return     "\n" +
           strprintf(_("Obsidian Team since 2016: EmsIsBack, Nephelli & Savagefury")) +
           "\n" +
           "\n" +
           strprintf(_("Official website: %s"),  URL_WEBSITE) +
           "\n" +
           "\n" +
           strprintf(_("Discord: %s"),  URL_DISCORD) +
           "\n" +
           "\n" +
           strprintf(_("Explorer: %s"),  URL_EXPLORER) +
           "\n" +
           "\n" +
           strprintf(_("Youtube: %s"),  URL_YOUTUBE) +
           "\n" +
           "\n" +
           "\n";
}
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (fs::exists(config_file_path)) {
        ...
        if (configFile != NULL) {
            std::string strHeader = "# Obsidian(OBS) config file:\n"
                                    ...
                                    "\n"
                                    "# Addnodes:\n"
                                    "addnode=78.141.237.229:2333\n"
                                    "addnode=82.33.128.177:2333\n"
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if obsidian is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if obsidian is started while in a temporary directory.\n",
