...
* Update manpages, see [gen-manpages.sh](https://github.com/ObsidianCoreCoin/obsidiancoin/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
### Obsidian maintainers/release engineers, suggestion for writing release notes
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in obsidian, then:

    pushd ./gitian-builder
    make -C ../obsidian/depends download SOURCES_PATH=`pwd`/cache/common
    popd
...
NOTE: Offline builds must use the --url flag to ensure Gitian fetches only from local URLs. For example:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --url obsidian=/path/to/obsidian,signature=/path/to/sigs {rest of arguments}
    popd

The gbuild invocations below <b>DO NOT DO THIS</b> by default.

### Build and sign Obsidian Network for Linux, Windows, and macOS:
...
Build output expected:
  ...
  1. source tarball (`obsidian-${VERSION}.tar.gz`)
...
Add other gitian builders keys to your gpg keyring, and/or refresh keys: See `../obsidian/contrib/gitian-keys/README.md`.
...
Codesigner only: Sign the macOS binary:
    ...
    transfer obsidian-osx-unsigned.tar.gz to macOS for signing
    tar xf obsidian-osx-unsigned.tar.gz
...
Codesigner only: Sign the windows binaries:
    ...
    tar xf obsidian-win-unsigned.tar.gz
...
- Detached signatures will then be committed to the [obsidian-detached-sigs](https://github.com/ObsidianCoreCoin/obsidiancoin-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
```
obsidian-${VERSION}-aarch64-linux-gnu.tar.gz
obsidian-${VERSION}-arm-linux-gnueabihf.tar.gz
obsidian-${VERSION}-i686-pc-linux-gnu.tar.gz
obsidian-${VERSION}-x86_64-linux-gnu.tar.gz
obsidian-${VERSION}-osx64.tar.gz
obsidian-${VERSION}-osx.dmg
obsidian-${VERSION}.tar.gz
obsidian-${VERSION}-win32-setup.exe
obsidian-${VERSION}-win32.zip
obsidian-${VERSION}-win64-setup.exe
obsidian-${VERSION}-win64.zip
```
The `*-debug*` files generated by the gitian build contain debug symbols
for troubleshooting by developers. It is assumed that anyone that is interested
in debugging can run gitian to generate the files for themselves. To avoid
end-user confusion about which file to pick, as well as save storage
space *do not upload these to the obsidiancoin.org server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the obsidiancoin.org server.

```
- Update obsidiancoin.org version

- Update other repositories and websites for new version

- Announce the release:

  - obsidian-dev mailing list

  - blog.obsidiancoin.org blog post

  - Update title of #obsidian and #obsidian-dev on Freenode IRC

  - Optionally twitter, reddit /r/Obsidian, ... but this will usually sort out itself

  - Archive release notes for the new version to `doc/release-notes/` (branch `master` and branch of the release)

  - Create a [new GitHub release](https://github.com/ObsidianCoreCoin/obsidiancoin/releases/new) with a link to the archived release notes.
