# PSBT Howto for Obsidian Network

Since Obsidian Network 0.17, an RPC interface exists for Partially Signed Obsidian
...
PSBT is an interchange format for Obsidian transactions that are not fully signed
...
Overall, the construction of a fully signed Obsidian transaction goes through the
following steps:
...
- An **Extractor** produces a valid Obsidian transaction (in network format)
...
#### Multisig with multiple Obsidian Network instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Obsidian Network. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `obsidian-cli` is used.
