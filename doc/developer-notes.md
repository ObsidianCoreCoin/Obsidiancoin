...
- [Development guidelines](#development-guidelines)
    - [General Obsidian Network](#general-obsidian-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Obsidian Network uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play obsidians" on the test network, if you
...
Obsidian Network is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ObsidianCoreCoin/obsidiancoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_obsidian
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_obsidian --log_level=test_suite
$ valgrind -v --leak-check=full src/obsidiand -printtoconsole
```
...
# A coverage report will now be accessible at `./test_obsidian.coverage/index.html`.
```
...
To profile a running obsidiand process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep obsidiand` -- sleep 60
```
...
Obsidian Network can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Obsidian Network, where everyone uses
...
pay attention to for reviewers of Obsidian Network code.

General Obsidian Network
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Obsidian Network uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Obsidian Network, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Obsidian Network so that they can be integrated
...
file descriptors it will cause problems with Obsidian's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Obsidian's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `obsidian-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `obsidian-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Obsidian Network, the wallet was always
